 ...
 *
 */
function houseCtrl($interval,$stateParams,$scope,$http,$interval)
{
		...
		$http(
		  {
		      method: 'POST',
		      url: '/api/outgoings/'+$i,
 		      data: "house_id="+$stateParams.id+"&date="+$scope.dates[$i]+"&concept="+$scope.concepts[$i]+"&outgoing="+$scope.outgoings[$i],
	...
	$scope.funch = function()
	{		
	 $http(
		  {
		      method: 'GET',
		      url: '/api/house/'+$stateParams.id
		  }
	    ).then(function(response){
...
function outGoingCtrl($stateParams,$scope,$http)
{
	$scope.date = "";
	$scope.concept = "";
	$scope.outgoing = "";
	$scope.add = function()
	{
		$http({
                           method: 'POST',
                           url: '/api/outgoings',
                           data: "house_id="+$stateParams.id+"&date="+$scope.date+"&concept="+$scope.concept+"&outgoing="+$scope.outgoing,
		           headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                       }).then(function(response){
...
function loginCtrl($scope,$http)
{
   this.ping = null;
		...
		this.ping = response.data.active;
		if(this.ping == true)
			      location.href='/#/';
	    });
           ...
           $scope.SignIn = function()
                   {
                       ...
                       $http({
                           method: 'POST',
                           url: '/api/auth',
                           data: "nick="+$scope.nick+"&pass="+$scope.pass,
		           headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                       }).then(function(response){
			     console.log(response.data);
			    if(response.data.state=="logged")
				{
				$scope.func();
                           	location.href = '/#/app/houses';
				}
			    else
        ...
        $scope.register = function()
                   {
                       ...
                       $http({
                           method: 'POST',
                           url: '/api/register',
                           data: "nick="+$scope.nick+"&name="+$scope.name+"&lastname="+
				 $scope.lastname+"&email="+$scope.email+"&pass="+$scope.pass1+
				 "&airbnb="+$scope.airbnb+"&airpass="+$scope.airpass,
		           headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                       }).then(function(response){
			    if(response.data.state=="logged")
				{
 ...
 */
function dashboardFlotOne($stateParams,$http,$scope) {
...
function flotChartCtrl($http,$scope,$stateParams) {
    ...
    $scope.outgoings = []; 
    $scope.Tearnings = []; 
     $scope.gUpdate = function()
	{	$http(
		  {
		      method: 'GET',
		      url: '/api/graph/'+$stateParams.id
		  }
	    ).then(function(response){
			response.data.earnings.forEach(function(entry) {
			    $scope.earnings.push([entry.date*1000,entry.total]);
			});
			response.data.outgoings.forEach(function(entry) {
			    $scope.outgoings.push([entry.date*1000,entry.total]);
			});
			response.data.earningsT.forEach(function(entry) {
			    $scope.Tearnings.push([entry.date*1000,parseFloat(entry.e)-parseFloat(entry.o)]);
			    console.log(entry);
			});
	    })};
$scope.gUpdate();
    function euroFormatter(v, axis) {
        return v.toFixed(axis.tickDecimals) + "MXN";
    }
    var position = 'right';
    ...
    this.flotMultiData2 = multiData2;
...
angular
  ...
  .controller('chartHouseCtrl', chartHouse)     
   .controller('outGoingCtrl', outGoingCtrl);chartHouse
